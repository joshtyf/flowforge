...
func (s *ServerHandler) registerRoutes(r *mux.Router) {
	...
	r.Handle("/api/service_request/{requestId}/logs/{stepName}", isAuthenticated(getOrgIdUsingSrId(s.mongoClient, isOrgMember(s.psqlClient, handleGetStepExecutionLogs(s.psqlClient))))).Methods("GET")
...
func handleGetStepExecutionLogs(psqlClient *sql.DB) http.Handler {
	type ResponseBody struct {
		StepName   string   `json:"step_name"`
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			logger.Error("[GetStepExecutionLogs] Unable to extract offset from query params", map[string]interface{}{"err": err})
			encode(w, r, http.StatusBadRequest, newHandlerError(ErrInvalidOffset, http.StatusBadRequest))
			return
		}
		if offset < 0 {
			logger.Error("[GetStepExecutionLogs] Invalid offset", map[string]interface{}{"offset": offset})
			encode(w, r, http.StatusBadRequest, newHandlerError(ErrInvalidOffset, http.StatusBadRequest))
			return
		}
		...
		if err != nil {
			logger.Error("[GetStepExecutionLogs] Unable to get log file", map[string]interface{}{"err": err})
			encode(w, r, http.StatusInternalServerError, newHandlerError(ErrInternalServerError, http.StatusInternalServerError))
			return
		}
		defer f.Close()
		...
		if err != nil {
			logger.Error("[GetStepExecutionLogs] Unable to scan log file", map[string]interface{}{"err": err})
			encode(w, r, http.StatusInternalServerError, newHandlerError(ErrInternalServerError, http.StatusInternalServerError))
			return
		}
		...
		if err != nil {
			logger.Error("[GetStepExecutionLogs] Unable to get latest event", map[string]interface{}{"err": err})
			encode(w, r, http.StatusInternalServerError, newHandlerError(ErrInternalServerError, http.StatusInternalServerError))
			return
		}
